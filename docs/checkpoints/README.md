# Checkpointy Countdown CRM

Checkpoint je neměnný historický záznam stavu k určitému datu. Neaktualizuje
se zpětně a nepřepisuje dnešní status.

## Aktuální autority

- Dnešní stav: [`../PROJECT_STATUS.md`](../PROJECT_STATUS.md)
- Pracovní proces: [`../WORKFLOW.md`](../WORKFLOW.md)
- Architektonický kontrakt: [`../architecture.md`](../architecture.md)
- Budoucí práce: [`../roadmap.md`](../roadmap.md)

## Dostupné historické důkazy

| Dokument | Účel |
|---|---|
| [`2026-09-01-documentation-consolidation.md`](2026-09-01-documentation-consolidation.md) | konsolidace autorit a workflow |
| [`2026-09-01-sandbox-call-order-rls-evidence.md`](2026-09-01-sandbox-call-order-rls-evidence.md) | autentizovaný Sandbox order, reload, SQL a negativní RLS důkaz |
| [`2026-09-01-sandbox-operator-browser-evidence.md`](2026-09-01-sandbox-operator-browser-evidence.md) | autentizovaný Sandbox Operator browser, reload a authorization evidence |
| [`../ADMIN_PILOT_PERSISTENCE_20260831.md`](../ADMIN_PILOT_PERSISTENCE_20260831.md) | Administrator browser + reload + SQL read-back |
| [`../RLS_ROLE_WORKSPACE_EVIDENCE_20260831.md`](../RLS_ROLE_WORKSPACE_EVIDENCE_20260831.md) | lokální rollback-scoped RLS testy |
| [`../PROJECT_POLISH_CHECKPOINT_20260826.md`](../PROJECT_POLISH_CHECKPOINT_20260826.md) | širší audit polish slices a otevřených nálezů |
| [`../AUDIT_CHECKPOINT_20260827.md`](../AUDIT_CHECKPOINT_20260827.md) | auditní a bezpečnostní kontrola z 27. 8. |
| [`../AKTUALNI_STAV_A_DESATERO.md`](../AKTUALNI_STAV_A_DESATERO.md) | starší stavový snapshot; není dnešní source of truth |
| [`../PRODUCT_STATUS.md`](../PRODUCT_STATUS.md) | starší auditní baseline; není dnešní source of truth |

Historické dokumenty mohou obsahovat staré počty testů, commity nebo doporučení.
Při konfliktu má přednost aktuální checkout, Git stav a `PROJECT_STATUS.md`.
