-- Keep the hardened idempotent completion boundary usable after a call has
-- moved from in_progress to awaiting_outcome. Permissions remain unchanged:
-- this legacy helper was revoked from Data API callers by the later hardening
-- migrations and is reached only by the authorized completion boundary.
...
BEGIN
  ...
  IF has_order THEN
    FOR item IN SELECT * FROM jsonb_to_recordset(order_items) AS order_item(product_id UUID, quantity INTEGER, unit_price NUMERIC)
    LOOP
      IF item.product_id IS NULL THEN RAISE EXCEPTION 'Order item product is required'; END IF;
      ...
      IF NOT FOUND THEN RAISE EXCEPTION 'Order item product is unavailable in the active workspace'; END IF;
      IF first_product_id IS NULL THEN first_product_id := product.id; END IF;
  ...
  completion := private.complete_lead_call_impl(
    ...
    call_ai_sentiment, first_product_id,
  ...
  IF completed_call_id IS NULL THEN RAISE EXCEPTION 'Call completion did not return a call ID'; END IF;
  ...
  IF has_order THEN
    ...
    LOOP
      ...
      INSERT INTO public.order_items (
        workspace_id, order_id, product_id, product_title_snapshot, unit_price,
        minimum_unit_price, quantity, line_total, currency
      ) VALUES (
        active_workspace_id, created_order_id, product.id, product.title,
        round(item.unit_price, 2), round(product.price, 2), item.quantity,
