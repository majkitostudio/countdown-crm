    ...
    <div className="bg-zinc-900 border border-zinc-800/80 hover:border-zinc-700 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all flex flex-col group">
      ...
      <div className="relative h-48 w-full bg-zinc-950 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wider border backdrop-blur-md ${getCategoryBadge(product.category)}`}>
            {product.category}
          </span>
        </div>
        ...
        <div className="absolute top-3 right-3">
          {product.in_stock ? (
            <span className="px-2.5 py-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-full text-[11px] font-medium flex items-center gap-1 backdrop-blur-md">
              <CheckCircle className="w-3 h-3" />
              In Stock ({product.stock_count ?? 50})
            </span>
          ) : (
            <span className="px-2.5 py-1 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-full text-[11px] font-medium flex items-center gap-1 backdrop-blur-md">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <h3 className="font-bold text-zinc-100 text-base group-hover:text-emerald-400 transition-colors line-clamp-1">
            {product.title}
          </h3>
        ...
        <div className="pt-2 border-t border-zinc-800/80 flex items-center justify-between text-xs">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-amber-400 hover:text-amber-300 font-medium px-2.5 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 transition-colors"
          >
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            onClick={() => onOpenObjections(product)}
            className="flex-1 py-2 px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-medium rounded-xl text-xs flex items-center justify-center gap-1.5 transition-colors border border-zinc-700"
          >
