...
import { Search, Bell, ShieldCheck, Layers, UserRound } from "lucide-react";
    ...
    <header className="h-18 border-b border-zinc-800/80 bg-zinc-950/80 backdrop-blur-md px-6 sm:px-8 flex items-center justify-between sticky top-0 z-20">
      {compact ? (
        <div className="flex items-center">
          <span className="text-base sm:text-lg font-semibold tracking-tight text-zinc-100">Operator Console</span>
        </div>
      ) : (
        <button
          onClick={() => {
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        {!compact && <div className="hidden xl:block"><OperatorPresenceBadge /></div>}

        {/* Live Indicator */}
        {!compact && <div className="hidden md:flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900 border border-zinc-800 text-[11px] text-zinc-400">
        ...
        {compact && (
          <time className="hidden sm:block text-right text-[10px] leading-tight text-zinc-400">
        ...
        >
          <div className="w-8 h-8 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xs font-semibold text-zinc-200">
            {compact ? <UserRound className="w-4 h-4" aria-hidden="true" /> : initials}
          </div>
