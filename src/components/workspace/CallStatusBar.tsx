...
export function CallStatusBar({ status, isCallActive, isDialing, activeLeadName, activeLeadPhone, onToggleCall, onSimulateIncoming, onStatusChange, onCallOutcome }: CallStatusBarProps) {
  ...
  return (
    <section className="p-5 rounded-xl bg-zinc-900/60 border border-zinc-800/80 flex flex-col lg:flex-row lg:items-center justify-between gap-5 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2 justify-end">
        <button onClick={onToggleCall} disabled={isDialing} className={`px-5 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all shadow-sm ${isCallActive ? "bg-rose-900/80 border border-rose-800 text-rose-100 hover:bg-rose-900" : "bg-zinc-100 text-zinc-950 hover:bg-zinc-200"}`}>
        ...
        {isCallActive && <>
          <button onClick={() => setIsMuted((value) => !value)} aria-label={isMuted ? "Unmute microphone" : "Mute microphone"} title={isMuted ? "Unmute microphone" : "Mute microphone"} className="p-2.5 rounded-xl border bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-zinc-200">{isMuted ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}</button>
          <button onClick={() => setIsOnHold((value) => !value)} aria-label={isOnHold ? "Resume call" : "Put call on hold"} title={isOnHold ? "Resume call" : "Put call on hold"} className="p-2.5 rounded-xl border bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-zinc-200">{isOnHold ? <Play className="w-4 h-4" /> : <Pause className="w-4 h-4" />}</button>
