    ...
    <div className="space-y-8 max-w-screen-2xl mx-auto">
      ...
      <div className="p-8 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 border-t border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-zinc-950 text-xs font-bold rounded-xl transition-all shadow-lg shadow-amber-500/10 cursor-pointer"
          >
      ...
      <div className="flex flex-wrap items-center gap-2 p-4 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl text-xs">
        ...
        {schema.attributes.map((attr) => (
          ...
          >
            {attr.name} <span className="text-amber-400 font-mono">({attr.type})</span>
          </span>
        ))}
      </div>
                      ...
                      return (
                        <td key={attr.id} className="py-3.5 px-4">
                          {attr.type === "number" ? (
                            ...
                            <span className="px-2 py-0.5 bg-zinc-800 text-amber-300 rounded font-semibold text-[10px]">
                              {String(val || "N/A")}
                            </span>
                          ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl p-6 shadow-2xl space-y-5 animate-in fade-in duration-200">
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-zinc-800">
              <button
                onClick={() => setIsAddModalOpen(false)}
                className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 text-xs font-medium rounded-xl border border-zinc-800"
              >
                Zrušit
              </button>
              <button
                onClick={handleAddRecord}
                className="px-5 py-2 bg-amber-500 hover:bg-amber-400 text-zinc-950 text-xs font-bold rounded-xl transition-colors"
              >
                Uložit záznam
              </button>
            </div>
