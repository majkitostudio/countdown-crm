...
export async function generateFollowupAction(
  ...
  if (operatorProfileError) {
    throw new Error("Operator profile lookup failed");
  }

  const typedOperatorProfile = operatorProfile as Pick<Database["public"]["Tables"]["profiles"]["Row"], "full_name"> | null;
  const operatorName = typedOperatorProfile?.full_name?.trim() || "Unknown operator";
  ...
  if (channel === "email") {
    ...
    switch (goal) {
      case "order_paylink":
        content = `Dobrý den ${leadName}, děkuji za hovor! Posílám Vám slíbený odkaz na objednávku ${productName || "balíčku"}: ${paylinkUrl} Přeji pěkný den! ${operatorName}, Countdown CRM`;
        break;

      case "discount_offer":
        content = `Dobrý den ${leadName}, posílám Vám speciální VIP slevu 15 % na ${productName || "vybraný balíček"}. Dokončit slevovou objednávku můžete zde: ${paylinkUrl} Platí 48h! ${operatorName}`;
        break;

      case "callback_reminder":
        content = `Zdravím ${leadName}, děkuji za domluvu hovoru! V kalendáři mám zapsanou naši schůzku. V případě potřeby změny mi dejte vědět. ${operatorName}`;
        break;

      case "graceful_thanks":
      default:
        content = `Dobrý den ${leadName}, děkuji za dnešní rozhovor a Váš čas. Přeji hezký zbytek dne! ${operatorName}, Countdown CRM`;
        break;
    }
  }
