  ...
  return (
    <div className="mx-auto max-w-screen-2xl space-y-8">
      <PageHeader
        ...
        actions={<Link href="/orders" className={getButtonClassName("secondary")}>View orders</Link>}
      />
      ...
      {data.canManage && balancesAvailable && data.balances.length > 0 && (
        <Surface variant="page">
          ...
          <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-3">{data.balances.map((balance) => <Surface key={balance.user_id} variant="inset" className="p-4"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-zinc-200">{balance.user_name}</p><p className="mt-1 text-[10px] text-zinc-600">{balance.transaction_count} transactions</p></div><p className={`font-mono text-sm font-semibold ${balance.balance >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{formatAmount(balance.balance, presentationCurrency)}</p></div></Surface>)}</div>
        </Surface>
      )}
      ...
      <Surface variant="page">
        <div className="flex flex-col gap-2 border-b border-zinc-800/80 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold text-zinc-100">Transaction ledger</h2>
            <p className="mt-1 text-xs text-zinc-500">Every entry is immutable and linked to its source or audit record.</p>
          </div>
          <span className="text-[10px] font-mono text-zinc-600">{data.transactions.length} entries</span>
        </div>
        {transactionsAvailable ? (
          data.transactions.length === 0 ? (
            <div className="p-12 text-center">
              <WalletCards className="mx-auto mb-4 h-8 w-8 text-zinc-600" />
              <h3 className="text-sm font-semibold text-zinc-200">No wallet transactions yet</h3>
              <p className="mx-auto mt-2 max-w-sm text-xs leading-relaxed text-zinc-500">
                Delivered order bonuses, monthly commissions and audited adjustments will appear here.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full min-w-[860px] text-left text-xs text-zinc-300">
                ...
                <tbody className="divide-y divide-zinc-800/60">
                  {data.transactions.map((transaction) => (
                    <tr key={transaction.id} className="hover:bg-zinc-800/30">
                      <td className="whitespace-nowrap px-5 py-4 text-zinc-500">{formatDate(transaction.created_at)}</td>
                      <td className="px-5 py-4 font-medium text-zinc-200">{transaction.user_name}</td>
                      <td className="px-5 py-4">
                        <span className="rounded-md border border-zinc-700 bg-zinc-950 px-2 py-1 text-[10px] text-zinc-300">
                          {transactionLabel(transaction.transaction_type)}
                        </span>
                      </td>
                      <td className="max-w-[320px] px-5 py-4 text-zinc-400">{transaction.reason}</td>
                      <td className="px-5 py-4 font-mono text-[10px] text-zinc-600">
                        {transaction.source_order_id
                          ? `order:${transaction.source_order_id.slice(0, 8)}`
                          : transaction.source_period_start || "manual"}
                      </td>
                      <td className={`whitespace-nowrap px-5 py-4 text-right font-mono font-semibold ${transaction.amount >= 0 ? "text-emerald-300" : "text-rose-300"}`}>
          ...
          <div className="p-5">
            <SectionWarning title="Transaction ledger unavailable" state={data.sections.transactions} />
          </div>
        )}
      </Surface>
    </div>
