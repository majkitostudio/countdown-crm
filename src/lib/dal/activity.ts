...
import {
  ...
  normalizeCustomerActivityEvents,
...
async function loadCustomerActivityEvents(
  ...
  const supabase = await createDataClient();
  async function readAllRows<T>(query: (offset: number) => PromiseLike<{ data: T[] | null; error: unknown }>): Promise<T[]> {
    const rows: T[] = [];
    for (;;) {
      const result = await query(rows.length);
      if (result.error) throw new DataAccessError("DATABASE", "Customer activity query failed");
      if (!result.data?.length) return rows;
      rows.push(...result.data);
    }
  }
  const [callData, orderData, noteData] = await Promise.all([
    readAllRows((offset) =>
    supabase
      .from("calls")
  ...
  const orders = orderData as unknown as OrderWithProduct[];
  const notes = noteData as LeadNoteRow[];
  ...
  const orderEvents: CustomerActivityEvent[] = orders.map((order) => {
    ...
    return {
      ...
      preview: {
        title: `Order recorded (${amount.toFixed(2)} ${order.currency || "USD"})`,
...
export async function listWorkspaceLeadActivityEvents(leadId: string): Promise<CustomerActivityEvent[]> {
  ...
  return normalizeCustomerActivityEvents(await loadCustomerActivityEvents(context.workspaceId, leadId));
}
...
export async function listWorkspaceLeadActivityPage(
  ...
  try {
    pageCustomerActivityEvents([], options);
    const events = await loadCustomerActivityEvents(context.workspaceId, leadId);
    return pageCustomerActivityEvents(events, options);
  } catch (error) {
    if (error instanceof Error && (error.message === "Invalid activity cursor" || error.message === "Invalid activity limit")) {
