...
function decodeCursorPayload(value: string): ActivityCursor {
  ...
  if (
    ...
    !Number.isFinite(Date.parse(decoded.occurred_at)) ||
    !decoded.id
  ) {
...
function decodeCursor(value: string): ActivityCursor {
  try { return decodeCursorPayload(value); }
  catch { throw new Error("Invalid activity cursor"); }
}
...
export function normalizeCustomerActivityEvents(events: CustomerActivityEvent[]): CustomerActivityEvent[] {
...
export function pageCustomerActivityEvents(
  ...
  const normalizedEvents = normalizeCustomerActivityEvents(events);
